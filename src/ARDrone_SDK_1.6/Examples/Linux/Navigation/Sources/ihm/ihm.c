...
printf("%f, %f, %f\n", ardrone_control_config_default.control_yaw, ardrone_control_config_default.control_vz_max, ardrone_control_config_default.euler_angle_max);
  ARDRONE_TOOL_CONFIGURATION_ADDEVENT(altitude_max, ardrone_control_config_default.altitude_max, NULL);
  ARDRONE_TOOL_CONFIGURATION_ADDEVENT(flight_without_shell, ardrone_control_config_default.flight_without_shell, NULL);
  ARDRONE_TOOL_CONFIGURATION_ADDEVENT(outdoor, ardrone_control_config_default.outdoor, NULL);
	ARDRONE_TOOL_CONFIGURATION_ADDEVENT(control_yaw, ardrone_control_config_default.control_yaw, NULL);
	ARDRONE_TOOL_CONFIGURATION_ADDEVENT(control_vz_max, ardrone_control_config_default.control_vz_max, NULL);
  ARDRONE_TOOL_CONFIGURATION_ADDEVENT(euler_angle_max, ardrone_control_config_default.euler_angle_max, NULL);
                          ...
                          gpointer    data )
{
  ...
  enemy_color = pos+1;
  ARDRONE_TOOL_CONFIGURATION_SET(enemy_colors, enemy_color);
                          ...
                          gpointer    data )
{
  gint pos;
  pos = gtk_combo_box_get_active( combo );
  ardrone_configuration_value_t v;
  v.i_value = pos;

  if (pos>=0 && pos<FLYING_MODE_NUM){
	  ARDRONE_TOOL_CONFIGURATION_SET(flying_mode, pos);
	  printf("Setting flight mode to %i.\n",v.i_value);
	  //ardrone_tool_configuration_addevent_flying_mode(v,NULL);
  }
}
                           ...
                           gpointer    data )
{
  ...
  ARDRONE_TOOL_CONFIGURATION_SET(detect_type,pos);

  // Enable or disable Cocarde control button depending on choice
  if ( (pos == CAD_TYPE_COCARDE) || (pos == CAD_TYPE_ORIENTED_COCARDE) )
    gtk_widget_set_sensitive(GTK_TOGGLE_BUTTON(button_roundel), TRUE);
  else
    gtk_widget_set_sensitive(GTK_TOGGLE_BUTTON(button_roundel), FALSE);
                           ...
                           gpointer    data )
{
  ...
  ARDRONE_TOOL_CONFIGURATION_SET(enemy_without_shell, flag );
}
                         ...
                         gpointer    data )
{
  ...
  if (!(pos<0 || pos>=ARDRONE_NB_ANIM_MAYDAY))
  {
	  ...
	  ARDRONE_TOOL_CONFIGURATION_SET(flight_anim,buffer);
  }
  else
  {
...
{
	static int toggled_navdata_demo = 0;
	toggled_navdata_demo ^= 1;
	ARDRONE_TOOL_CONFIGURATION_SET(navdata_demo,(int)toggled_navdata_demo);
}

static void ihm_toggle_cocarde_control( GtkComboBox *combo, gpointer data )
{
    static int toggled_cocarde_control = 0;
    toggled_cocarde_control ^= 1;
    ARDRONE_TOOL_CONFIGURATION_SET(flying_mode, (int)(toggled_cocarde_control ? FLYING_MODE_HOVER_ON_TOP_OF_ROUNDEL : FLYING_MODE_FREE_FLIGHT) );
}
