...
#define NB_STAGES 10
#define WINDOW_STREAM_FRONT "FrontCamera"

PIPELINE_HANDLE pipeline_handle;

IplImage *rgbHeader = NULL;/* stream */
IplImage *stream_front;
...
C_RESULT output_gtk_stage_open( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
  ...
  stream_front = cvCreateImage(cvSize(QVGA_WIDTH, QVGA_HEIGHT), IPL_DEPTH_8U, 3);
...
C_RESULT output_gtk_stage_transform( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
  ...
  rgbHeader->imageData = (char *)pixbuf_data;

  cvCvtColor(rgbHeader, stream_front, CV_RGB2BGR);
  cvShowImage(WINDOW_STREAM_FRONT, stream_front);
...
{
  cvDestroyWindow(WINDOW_STREAM_FRONT);
  cvReleaseImage(&stream_front);
  return (SUCCESS);
}
