...
reinflearn::reinflearn(bot_ardrone *bot)
{
	...
	if (restore) {
		...
		v2[0] = layout->width;
		v2[1] = layout->height;
		stage_l2r->ff->add_rect_forces(v1, v2, 0.5, 5);
		stage_r2l->ff->add_rect_forces(v1, v2, 0.5, 5);
		// - pylon1
		real2sim(layout->pylon1, v1);
		stage_l2r->ff->add_circle_forces(v1, layout->pylonr, 0.5, 10);
		stage_r2l->ff->add_circle_forces(v1, layout->pylonr, 0.5, 10);
		stage_l2r->ff->add_spiral_forces(v1, layout->pylonr, 0.5, 10, PI/2);
		stage_r2l->ff->add_spiral_forces(v1, layout->pylonr, 0.5, 10, PI/2);
		// - pylon2
		real2sim(layout->pylon2, v1);
		stage_l2r->ff->add_circle_forces(v1, layout->pylonr, 0.5, 10);
		stage_r2l->ff->add_circle_forces(v1, layout->pylonr, 0.5, 10);
