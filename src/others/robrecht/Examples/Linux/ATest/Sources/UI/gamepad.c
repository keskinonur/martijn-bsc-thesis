...
////////////////////////////////////
// keyboard control           	  //
// Run steuerung as root          //
////////////////////////////////////


C_RESULT open_autonom(void)
{
  unlink("/tmp/autonom_flight");
  //char c;
  //c = getchar();
  chmod("/tmp/autonom_flight", 00777);
  if((mkfifo("/tmp/autonom_flight", O_RDWR)) == -1)
  {
    fprintf(stderr, "Can not create a FIFO.........\n");
    return C_FAIL;
  }
...
C_RESULT update_autonom(void)
{
  ...
  if((fd_fifo=open("/tmp/autonom_flight", O_RDWR)) == - 1)
  {
  ...
  if(read(fd_fifo, &buf, sizeof(buf)) == -1)
    ...
    {	printf("Record mode Activated");
      recordMode();

    }else if( strcmp(buf, "fw") == 0)
    {
      printf("I fly forward\n");
      ardrone_at_set_progress_cmd(1, 0, (-1*speedval), 0, 0);
    }
    else if( strcmp(buf, "bw") == 0)
    {
      printf("I fly backward\n");
      ardrone_at_set_progress_cmd(1, 0, speedval, 0, 0);
    }
    else if( strcmp(buf, "ri") == 0)
    {
      printf("I fly to the right side\n");
      ardrone_at_set_progress_cmd(1, speedval, 0, 0, 0);
    }
    else if( strcmp(buf, "le") == 0)
    {
      printf("I fly to the left side\n");
      ardrone_at_set_progress_cmd(1, (-1*speedval), 0, 0, 0);
    }
    else if( strcmp(buf, "tl") == 0)
    {
      printf("I turn to the left side\n");
      ardrone_at_set_progress_cmd(1, 0, 0, 0, (-0.5));
    }
    else if( strcmp(buf, "tr") == 0)
    {
      printf("I turn to the right side\n");
      ardrone_at_set_progress_cmd(1, 0, 0, 0, 0.5);
    }
    ...
    else if( strcmp(buf, "si") == 0)
    {
      ...
      if(speedval<1) speedval +=0.1;
    }
    else if( strcmp(buf, "sd") == 0)
    {
      printf("Decrease Speed");
      if(speedval>0) speedval -=0.1;
    }
...
C_RESULT update_radioGP(void)
{
  ...
  // Buffer d�composition in blocs (if the last is incomplete, it's ignored)
  int32_t idx = 0;
